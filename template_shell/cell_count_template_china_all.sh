#!/bin/sh
echo "----------------------------------------------create table cell_all_property_from_cdrmm_count_template_city_china_all !!!!!!!!!!!!!!!!!!!!!!!!!!!!"
hive -e "create table cell_all_property_from_cdrmm_count_template_city_china_all(
lacstart int,
cistart int,
cdrmm_length float,
...
echo "------------------------------------------------generate cell_all_property_from_cdrmm_count_template_city_china_all start!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
...
insert overwrite table cell_all_property_from_cdrmm_count_template_city_china_all
...
count(case when a.FDD_LTE=1 and a.net='2G' then 1 else null end) as FDDfall2g_cdrmm_times
 group by nvl(a.lacstart,-1),nvl(a.cistart,-1);"
echo "------------------------------------------------generate cell_all_property_from_cdrmm_count_template_city_china_all finish!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
echo "----------------------------------------------create table cell_all_property_from_url_count_template_city_china_all !!!!!!!!!!!!!!!!!!!!!!!!!!!!"
hive -e "create table cell_all_property_from_url_count_template_city_china_all(
lacstart int,
cistart int,
...
row format delimited fields terminated by ',' stored as TEXTFILE;"
echo "------------------------------------------------generate cell_all_property_from_url_count_template_city_china_all start!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
...
insert overwrite table cell_all_property_from_url_count_template_city_china_all
...
sum(nvl(a.download,0))/1048576+sum(nvl(a.upload,0))/1048576 as cistart_url_size  
 group by nvl(a.lacstart,-1),nvl(a.cistart,-1);"
echo "------------------------------------------------generate cell_all_property_from_url_count_template_city_china_allu finish!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
echo "----------------------------------------------create table cell_all_property_from_cdrmmandurl_count_template_city_china_all !!!!!!!!!!!!!!!!!!!!!!!!!!!!"
hive -e "create table cell_all_property_from_cdrmmandurl_count_template_city_china_all(
lacstart int,
cistart int,
NET             STRING                        ,
PROVIN             STRING                        ,
...
row format delimited fields terminated by ',' stored as TEXTFILE;"
echo "------------------------------------------------generate cell_all_property_from_cdrmmandurl_count_template_city_china_all start!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
hive -e "from (select 
...
insert overwrite table cell_all_property_from_cdrmmandurl_count_template_city_china_all
...
count(distinct case when t.package_level_score=10 then t.callingmsisdn else null end) as low_package_num
 group by nvl(t.lacstart,-1),nvl(t.cistart,-1);"
echo "------------------------------------------------cell_all_property_from_cdrmmandurl_count_template_city_china_all finish!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
echo "----------------------------------------------create table cell_all_property_from_url_count_for_user_template_city_china_all !!!!!!!!!!!!!!!!!!!!!!!!!!!!"
hive -e "create table cell_all_property_from_url_count_for_user_template_city_china_all(
lacstart int,
cistart int,
user_most_url_size   float
)
row format delimited fields terminated by ',' stored as TEXTFILE;"
echo "------------------------------------------------generate cell_all_property_from_url_count_for_user_template_city_china_all start!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
hive -e "from (select nvl(a.lacstart,-1) as lacstart,nvl(a.cistart,-1) as cistart,nvl(a.callingmsisdn,'-1') as msisdn,sum(nvl(a.download,0))/1048576+sum(nvl(a.upload,0))/1048576 as  user_most_url_size from handle_url_template_city a group by nvl(a.lacstart,-1),nvl(a.cistart,-1),nvl(a.callingmsisdn,'-1') ) tt 
insert overwrite table cell_all_property_from_url_count_for_user_template_city_china_all
select tt.lacstart,tt.cistart,max(tt.user_most_url_size) group by tt.lacstart,tt.cistart;"
echo "------------------------------------------------generate cell_all_property_from_url_count_for_user_template_city_china_all finish!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"

echo "----------------------------------------------create table cell_property_from_user_count_template_city_china_all !!!!!!!!!!!!!!!!!!!!!!!!!!!!"
hive -e "create table cell_property_from_user_count_template_city_china_all(
lacstart int,
cistart int,
num_1_10   int,
num_10_50   int,
...
echo "------------------------------------------------generate cell_property_from_user_count_template_city_china_all start!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
hive -e "from (select nvl(lacstart,-1) as lacstart,nvl(cistart,-1) as cistart,callingmsisdn,provin,citynet from handle_cdrmm_template_city  union all select nvl(lacstart,-1) as lacstart,nvl(cistart,-1) as cistart,callingmsisdn,provin,citynet from handle_url_template_city ) b 
left join user_count_all_result_template_city_china_all a on (a.msisdn=b.callingmsisdn) left join cell_all_property_from_url_count_template_city_china_all c on (b.lacstart=c.lacstart and b.cistart=c.cistart) left join cell_all_property_from_url_count_for_user_template_city_china_all d on (d.lacstart=b.lacstart and d.cistart=b.cistart)
insert overwrite table cell_property_from_user_count_template_city_china_all
...
echo "------------------------------------------------generate cell_property_from_user_count_template_city_china_all finish!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
echo "----------------------------------------------create table cell_charge_template_city_china_all !!!!!!!!!!!!!!!!!!!!!!!!!!!!"
hive -e "create table cell_charge_template_city_china_all(
lacstart int,
cistart int,
...
echo "------------------------------------------------generate cell_charge_template_city_china_all start!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
hive -e "from 
(
...
on (c.msisdn=d.callingmsisdn) group by nvl(d.lacstart,-1),nvl(d.cistart,-1)  
) bb 
on (aa.lacstart=bb.lacstart and aa.cistart=bb.cistart) 
insert overwrite table cell_charge_template_city_china_all
select 
...
echo "------------------------------------------------generate cell_charge_template_city_china_all finish!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
echo "----------------------------------------------create table cell_package_terminal_template_city_china_all !!!!!!!!!!!!!!!!!!!!!!!!!!!!"
hive -e "create table cell_package_terminal_template_city_china_all(
lacstart int,
cistart int,
cistart_package_score float,
cistart_terminal_score float 
) row format delimited fields terminated by ','  stored as TEXTFILE;"
echo "------------------------------------------------generate cell_package_terminal_template_city_china_all start!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
...
from (select callingmsisdn,lacstart,cistart,package_level_score,terminal_level_score from handle_cdrmm_template_city  union all select callingmsisdn,lacstart,cistart,package_level_score,terminal_level_score from handle_url_template_city ) t 
group by t.lacstart,t.cistart,t.callingmsisdn) tt
insert overwrite table cell_package_terminal_template_city_china_all
...
echo "------------------------------------------------generate cell_package_terminal_template_city_china_all finish!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
echo "----------------------------------------------create table cell_business_url_times_template_city_china_all !!!!!!!!!!!!!!!!!!!!!!!!!!!!"
hive -e "create table cell_business_url_times_template_city_china_all(
lacstart int,
cistart int,
...
echo "------------------------------------------------generate cell_business_url_times_template_city_china_all start!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
#hive -e ""
echo "------------------------------------------------generate cell_business_url_times_template_city_china_all finish!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
echo "----------------------------------------------create table cell_business_url_traffic_template_city_china_all !!!!!!!!!!!!!!!!!!!!!!!!!!!!"
hive -e "create table cell_business_url_traffic_template_city_china_all (
lacstart int,
cistart int,
business_type string,
total_dl float,
...
echo "------------------------------------------------generate cell_business_url_traffic_template_city_china_all start!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
#hive -e ""
echo "------------------------------------------------generate cell_business_url_traffic_template_city_china_all finish!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
echo "----------------------------------------------create table cell_count_all_result_template_city_china_all !!!!!!!!!!!!!!!!!!!!!!!!!!!!"
hive -e "create table cell_count_all_result_template_city_china_all(
 lacstart           int                   ,
 cistart             int                        ,
 NET             STRING                        ,
 PROVIN             STRING                        ,
 CITY               STRING                ,
cistart_cdrmm_length float,
cistart_url_size   float,
cistart_charge float
) row format delimited fields terminated by ','  stored as TEXTFILE;"
echo "------------------------------------------------generate cell_count_all_result_template_city_china_all start!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
hive -e "from  (select nvl(t.lacstart,-1) as lacstart,nvl(t.cistart,-1) as cistart from (select lacstart,cistart from handle_cdrmm_template_city  union all select lacstart,cistart from handle_url_template_city ) t group by nvl(t.lacstart,-1),nvl(t.cistart,-1)) tt 
left join cell_property_from_user_count_template_city_china_all a on (tt.lacstart=a.lacstart and tt.cistart=a.cistart)
left join cell_all_property_from_cdrmmandurl_count_template_city_china_all b on (tt.lacstart=b.lacstart and tt.cistart=b.cistart)
left join cell_all_property_from_cdrmm_count_template_city_china_all c on (tt.lacstart=c.lacstart and tt.cistart=c.cistart)
left join cell_all_property_from_url_count_template_city_china_all d on (tt.lacstart=d.lacstart and tt.cistart=d.cistart)
left join cell_business_url_traffic_template_city_china_all e on (tt.lacstart=e.lacstart and tt.cistart=e.cistart)
left join cell_business_url_times_template_city_china_all f on (tt.lacstart=f.lacstart and tt.cistart=f.cistart)
left join cell_charge_template_city_china_all h on (tt.lacstart=h.lacstart and tt.cistart=h.cistart)
left join cell_package_terminal_template_city_china_all i on (tt.lacstart=i.lacstart and tt.cistart=i.cistart)
insert overwrite table cell_count_all_result_template_city_china_all
select b.lacstart,
b.cistart,
b.NET,
b.PROVIN,
...
d.cistart_url_size,
h.cistart_charge 
order by c.cdrmm_length desc;"
echo "------------------------------------------------generate cell_count_all_result_template_city_china_all finish!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
